...
static uint8_t generateDelayBasedOnCAName( const uint8_t* name, uint8_t tickMs )
{
	uint16_t result = 0u;
...
static void handleCannotClaimAddress( void )
{
	if ( acl.wasRequestForACLReceived )
	{
		acl.pseudoDelay = generateDelayBasedOnCAName( acl.caName, acl.tickMs );
...
void updateAddressClaimed( void )
{
	switch ( acl.state )
	{
		...
		{
			handleCannotClaimAddress( );
			break;
		}
		case DELAY_BEFORE_CANNOT_CLAIM:
		{
			handleDelayAndTransitionTo( CANNOT_CLAIM_ADDRESS );
			break;
		}
		case NORMAL_TRAFFIC:
		{
